...
Server.prototype = {
  ...
  _createBundleCode: function(reactCode, appCode) {
    switch (this.webpackConfig.devtool) {
      case 'source-map':
      case 'cheap-module-source-map':
      case 'cheap-source-map':
      ...
      default:
        return reactCode.replace(SOURCEMAP_REGEX, '//# sourceMappingURL=' + this.reactMapURL) +
          '\n' + appCode
    }
  },
  ...
  _startPackageServer: function() {
    ...
    var cmd = require.resolve('react-native/packager/packager.sh');
    var args = [
      '--root', path.join(require.resolve('react-native/package.json'), '..'),
      '--root', this.entryDir,
